...
namespace Client {
    sealed class ModifierMultiplicativeSystem : MainEcsSystem {
        readonly EcsWorldInject _world = default;
        readonly EcsFilterInject<Inc<DamageModifier, MultiplicativeModifier>> _filter = default;
        ...
        {
            return new ModifierMultiplicativeSystem();
        }
            ...
            foreach(var entity in _filter.Value)
            {
                ...
                for (int i = 0; i < resolveComp.Components.Count; i++)
                {
                    if(resolveComp.Components[i] is DamageEffect effect)
                    {
                        ...
                        effect.MultiplicativeModifiers += modifierComp.Modifier.Value;
