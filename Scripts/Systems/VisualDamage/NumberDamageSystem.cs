...
namespace Client {
    sealed class NumberDamageSystem : MainEcsSystem
    {
        readonly EcsWorldInject _world = default;
        readonly EcsFilterInject<Inc<TakeDamageComponent, NumberDamageAllowedComponent>, Exc<ConditionTakeDamageComponent>> _filter;
        ...
        public override void Run (IEcsSystems systems) {
            foreach (var entity in _filter.Value)
            {
                ...
                if(takeDamageComp.TargetEntity.Unpack(_world.Value, out int targetEntity))
                {
                    ...
                    {
                        _visualDamageConfig.PhysicalDamageEnemy.Spawn(positionSpawn, takeDamageComp.Damage);
                    }
                    else
                    {
                        _visualDamageConfig.PhysicalDamagePlayer.Spawn(positionSpawn, takeDamageComp.Damage);
                    }
                }
            }
        }
